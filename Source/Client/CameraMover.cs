...
public struct CameraMover : IComponent
{
    ...
    public readonly void Update(UpdateArgs args)
    {
        ...
        if (args.State.IsDown(Key.W))
            direction -= Vector3.UnitZ;
        if (args.State.IsDown(Key.S))
            direction += Vector3.UnitZ;
        if (args.State.IsDown(Key.D))
            direction += Vector3.UnitX;
        if (args.State.IsDown(Key.A))
            direction -= Vector3.UnitX;
        if (args.State.IsDown(Key.LeftShift))
            direction += Vector3.UnitY;
        if (args.State.IsDown(Key.LeftControl))
            direction -= Vector3.UnitY;
        ...
        float xRot = 0;
        if (args.State.IsDown(Key.E))
            xRot -= 0.004f;
        if (args.State.IsDown(Key.Q))
            xRot += 0.004f;
        ...
        args.EventsQueue = new([]);
        // if (args.Input.MouseDelta != Vector2.Zero)
        // {
