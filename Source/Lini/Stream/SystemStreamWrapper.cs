...
public sealed class SystemStreamWrapper : IReadWriteSeekStream<byte>
{
    private System.IO.Stream Wrapped { get; init; }
    public nuint Position
    {
        get => (nuint)Wrapped.Position;
        set
        {
            if (!Wrapped.CanSeek)
                throw new NotSupportedException("Cannot seek in a system stream which does not support seeking.");

            Wrapped.Seek((long)value, SeekOrigin.Begin);
        }
    }

    public nuint Length => (nuint)Wrapped.Length;
    ...
    public static IReadWriteStream<byte> WrapReadWrite(System.IO.Stream stream)
    {
    ...
    public static IReadWriteSeekStream<byte> WrapReadWriteSeek(System.IO.Stream stream)
    {
    ...
    public static IReadSeekStream<byte> WrapReadSeek(System.IO.Stream stream)
    {
    ...
    public static IWriteSeekStream<byte> WrapWriteSeek(System.IO.Stream stream)
    {
